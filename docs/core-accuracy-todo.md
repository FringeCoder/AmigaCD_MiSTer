...
**Partly instrumented 2026-09-15, without hardware.** `[SUITE]` meant a MiSTer,
a screen and someone reading it, which is why this sat. Two instruments now
exist, both built on [Copperline](https://github.com/CopperlineHQ/Copperline), a
separately written cycle-driven Amiga emulator with a machine-readable control
protocol. It is not silicon, so it is a second opinion and not ground truth —
but it is one a machine can read.

- **In CI now.** `rtl/sim/beamcounter/tb_beamcounter_reference.sv` against
  `beam-reference.tsv`, generated by `tools/copperline-beamtable.sh`. It checks
  the horizontal readback offset at every colour clock of two frames, and that
  the two halves of the vertical readback describe one line. Falsified two ways:
  dropping the `06f30af` decrement, and reading VPOSR from the live counter
  while VHPOSR reads `vpos_rb`.

  Its first version checked the invariants only at the table's 175 sampled
  positions and **passed against that second mutation** — none of those
  positions lands where the lag is observable. The checking is dense now and the
  table supplies comparison points only. Worth remembering: the table made the
  bench look thorough while it was testing nothing.

- **Needs the machine.** `scripts/timing-probe.sh` in the userspace repo, with
  `docs/timing-probe.md`. Copperline's `timing-test` probes stream their results
  out the SERIAL port, and this core has a UART, so the same disk can run on
  both and be diffed numerically. The Copperline column is captured and
  committed; the MiSTer half has never been run and rests on one untested
  assumption — that the core's serial output reaches a host device we can read.
  `timing-probe.sh diagnose` settles that first.

**First finding, recorded rather than acted on.** At the frame wrap Copperline's
VHPOSR has already advanced to line 0 while its VPOSR still carries `vpos[8]`
from line 312, so the pair reassembles to line 256 — a line the beam was never
on. Ours reads both halves from `vpos_rb` and reassembles to 312, the previous
line: lagging, but consistent. Which is right is exactly the sort of question
this item exists to ask, and neither implementation settles it. The bench pins
our behaviour so a sync cannot change it silently.

This does **not** close T7. Scoring the vAmigaTS VPOS suite is still the
measurement the item asks for, and the 36 failing rows are still uncounted here.
